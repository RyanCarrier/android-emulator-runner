...
export async function installAndroidSdk(
  ...
  try {
    ...
    await execWithRetry(() => exec.exec(`sh -c \\"sdkmanager --install 'system-images;android-${apiLevel};${target};${arch}' --channel=${channelId} > android-system-images.log"`), retryCount);
